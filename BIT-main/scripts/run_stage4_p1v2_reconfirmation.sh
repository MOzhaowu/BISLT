...
results_root="${BIT_STAGE4_P1V2_RECONFIRM_ROOT:-$workspace_dir/baseline/moped/stage4_p1v2_reconfirmation}"

run_case() {
    local object="$1"
    local sequence="$2"
    local seed="$3"
    echo "[P1V2-RECONFIRM] $object/$sequence seed=$seed"
    BIT_BASELINE_ROOT="$results_root" \
    BIT_CONFIG_GLOB="$object/evaluation/$sequence.yml" \
    BIT_RUN_TAG="p1v2_reconfirm_s$seed" \
...
for seed in 53 54 55 56 57 58; do
