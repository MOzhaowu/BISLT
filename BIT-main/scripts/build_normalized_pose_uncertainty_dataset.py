    ...
    for path in sorted(args.run_root.glob(args.run_glob)):
        ...
        for row in rows:
            ...
            record = {
                ...
                "visible_boundary_ratio": finite(row["visible_boundary_ratio"]),
                "occlusion_ratio": finite(row["occlusion_ratio"]),
                "effective_contour_ratio": finite(row["effective_contour_ratio"]),
                "silhouette_iou": finite(row["silhouette_iou"]),
                "contour_residual_stddev": finite(row["contour_residual_stddev"]),
                "contour_residual_p90": finite(row["contour_residual_p90"]),
                "contour_residual_centroid_offset": finite(row["contour_residual_centroid_offset"]),
                "pose_diagnostics_time_ms": finite(row["pose_diagnostics_time_ms"]),
                "hessian_diagnostics_time_ms": finite(row["hessian_diagnostics_time_ms"]),
                "contour_search_lines": int(row["contour_search_lines"]),
                "active_contour_lines": int(row["active_contour_lines"]),
                "matched_contour_lines": int(row["matched_contour_lines"]),
