...
namespace summer
{
	...
	TemplateView::TemplateView(Object3D *object, float alpha, float beta, float gamma, float distance, int numLevels, bool generateNeighbors)
	{
		...
		Mat mask0 = view->DownloadFrame(Renderer::MASK);
		Mat depth0 = view->DownloadFrame(Renderer::DEPTH);

		Matx33f K = view->K44s()[view->level()].get_minor<3, 3>(0, 0);
		...
		for (int level = 2; level < _numLevels; level++)
		{
			...
			Mat mask = view->DownloadFrame(Renderer::MASK);
			mask = mask(roi).clone();
