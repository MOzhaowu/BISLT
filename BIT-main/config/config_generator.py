...
def GenRbotYmls4Summer(scene, models, rbotRoot, saveRoot):
    ...
    for model in models:
        ...
        with open(yml_path, 'w') as yml_file:
            ...
            yml_file.write(f"model_validation:\n")
            yml_file.write(f"    enabled: True\n")
            yml_file.write(f"    validation_frames: 3\n")
            yml_file.write(f"    min_improvement: 0.01\n")
            yml_file.write(f"    rotation_delta_deg: 2.0\n")
            yml_file.write(f"    translation_delta: 0.005\n")
            yml_file.write(f"    max_iou_regression: 0.005\n")
            yml_file.write(f"    buffer_size: 32\n")
            yml_file.write(f"    selection:\n")
            yml_file.write(f"        rotation_weight: 1.0\n")
            yml_file.write(f"        translation_weight: 100.0\n")
            yml_file.write(f"        quality_weight: 0.25\n")
            yml_file.write(f"    weights:\n")
            yml_file.write(f"        iou: 1.0\n")
            yml_file.write(f"        pose_consistency: 0.25\n")
            yml_file.write(f"        temporal_stability: 0.25\n")
            yml_file.write(f"        uncertainty: 0.25\n\n")
