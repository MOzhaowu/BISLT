...
class Mesh(object):
    ...
    @classmethod
    def from_obj_with_normal(cls, filename_obj, normalization=False, load_texture=False, texture_res=1, texture_type='surface'):
        '''
        Create a Mesh object from a .obj file
        '''
        if load_texture:
            vertices, faces, face_normals, textures = srf.from_obj_with_normal(filename_obj,
                                                     normalization=normalization,
                                                     texture_res=texture_res,
                                                     load_texture=True,
                                                     texture_type=texture_type)
        else:
            vertices, faces, face_normals = srf.from_obj_with_normal(filename_obj,
                                           normalization=normalization,
                                           texture_res=texture_res,
                                           load_texture=False)
            textures = None
        return cls(vertices, faces, face_normals, textures, texture_res, texture_type)
