...
def load_obj_with_normal(filename_obj, normalization=False, load_texture=False, texture_res=4, texture_type='surface'):
    """
    Load Wavefront .obj file.
    This function only supports vertices (v x x x) and faces (f x x x).
    """
    ...
    if load_texture:
        return vertices, faces, face_normals, textures
    else:
...
def normalize_vertices(vertices):
    ...
    vertices_max = torch.abs(vertices).max()
    vertices = vertices / torch.abs(vertices).max()

    vertices *= 2 
    
    vertices_max_array = vertices.max(0)[0][None, :]/2
    vertices -= vertices_max_array
    return vertices, vertices_min, vertices_max, vertices_max_array


def denormalize_vertices(vertices_normalized, vertices_min, vertices_max, vertices_max_array):
    vertices_denormalized = vertices_normalized
    vertices_denormalized += vertices_max_array
    vertices_denormalized /= 2
    vertices_denormalized *= vertices_max
    vertices_denormalized += vertices_min
    return vertices_denormalized
