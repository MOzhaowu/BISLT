...
__global__ void forward_soft_rasterize_cuda_kernel(
    ...
    //////////////////////////////////////////////
if(1){
	// printf("func_id_alpha: %f %f", func_id_alpha, func_id_rgb);
...
std::vector<at::Tensor> forward_soft_rasterize_cuda(
    ...
    #if defined(TORCH_VERSION_MAJOR) && TORCH_VERSION_MAJOR >= 1 && TORCH_VERSION_MINOR >= 2
        auto faces_type = faces.scalar_type();
    #else
        auto faces_type = faces.type();  // no longer supported in new C++/CUDA extension of pytorch
    #endif
    AT_DISPATCH_FLOATING_TYPES(faces_type, "forward_soft_rasterize_inv_cuda", ([&] {
    ...
    AT_DISPATCH_FLOATING_TYPES(faces_type, "forward_eff_soft_rasterize_cuda", ([&] {
...
std::vector<at::Tensor> backward_soft_rasterize_cuda(
    ...
    const dim3 blocks ((batch_size * image_size * image_size - 1) / threads + 1);
        ...
        auto faces_type = faces.type();  // no longer supported in new C++/CUDA extension of pytorch
    #endif
    AT_DISPATCH_FLOATING_TYPES(faces_type, "backward_soft_rasterize_cuda", ([&] {
