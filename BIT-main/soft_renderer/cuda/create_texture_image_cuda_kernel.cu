...
at::Tensor create_texture_image_cuda(
    ...
    const dim3 blocks ((image_size / 3 - 1) / threads + 1);
        ...
        auto image_type = image.scalar_type();
    #else
        auto image_type = image.type();  // no longer supported in new C++/CUDA extension of pytorch
    #endif
    AT_DISPATCH_FLOATING_TYPES(image_type, "create_texture_image_cuda", ([&] {
